    ...
    <a-drawer :visible="isDrawerVisible" :width="800" @close="isDrawerVisible = false">
      <template #title>
        <div style="display: flex; justify-content: space-between; align-items: center">
          <span> {{ selectedLogDetail?.displayName }} ({{ selectedLogDetail?.filename }}) </span>
...
import { fetchHistoryLogs, fetchLogDetail, downloadLogFile } from '@/services/logService'
...
const formatDate = (date: dayjs.Dayjs) => (date ? date.format('MMM Do, YYYY') : '') // Example: Jul 24th, 2024
...
const updateHistoryLogs = async () => {
  try {
    ...
    histories.value = []
    $message.error('Failed to fetch history logs. Please try again later.')
    throw error
  }
}
...
const onReadLogDetail = async (history: LogEntry) => {
  try {
    ...
    $message.error('Error fetching log detail. Please try again later.')
    throw error
  }
}
...
const handleDownload = async () => {
  ...
  try {
    ...
    $message.error('Error downloading log file. Please try again later.')
    throw error
  }
}
